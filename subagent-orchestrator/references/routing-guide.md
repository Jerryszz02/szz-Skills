...
| --- | --- | --- |
| Known edit, short explanation, tightly coupled work with costly handoff/acceptance | Main manager | Name the concrete reason; reassess when scope or failures grow. |
| Waiting, receipts, approved patch application, fixed checks | Deterministic tools invoked by manager | Use `execution-flow.md`; no ceremonial model worker. |
...
| Source-writing implementation, tests, fixes, documentation | DeepSeek → Spark → Luna → Terra | DeepSeek stays first; Spark needs a fully specified small slice. Native writers use `medium`. |
| Additional independently useful mechanical integration that passes the gate | Spark (`medium`) → Luna (`medium`) → Terra (`medium`) → main manager | Fixed patch application stays deterministic; never bypass failed safety checks. |
...
2. For permitted source-writing execution, use `scripts/run-dsh-worker.sh`: it resolves the executable, checks headless help/config and the packet, and records `route.json` even for preflight rejection once a safe output directory exists. Follow [provider-diagnostics.md](provider-diagnostics.md); do not declare a CLI unavailable from a chained `command -v` check alone. DeepSeek retains its configured provider reasoning settings; native `low`/`medium` tiers do not apply to external CLIs.
3. If DeepSeek is unavailable or fails, diagnose the reason, then use eligible native Spark, Luna and Terra (`medium`) within budget. Skip Spark if its eligibility or runtime gate fails; missing native support never moves it ahead of DeepSeek. For an existing native writer's local acceptance failure, prefer its one targeted continuation before changing routes; see `execution-flow.md`. Check actual tool callability, not just catalog entries. Record skipped-route reasons.
4. Approved patch application and fixed checks use deterministic tools under manager ownership; read `execution-flow.md`. Additional independently useful mechanical integration can use eligible Spark (`medium`), then Luna (`medium`), Terra and manager after the gate. External runners cannot verify uncommitted integrated changes; no helper rejection authorizes bypassing checks through a worker.
