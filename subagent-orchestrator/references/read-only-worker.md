...
The manager checks rules/state and supplies the question, not a precomputed file map. Scout uncertainty is an acceptable result when supported by evidence; the manager can ask one targeted follow-up within budget. A new source repair requires an explicit writing packet and the writing route. Preserve cited findings so the executor does not restart exploration. A read-only worker stops when its bounded question is answered, returns its evidence promptly and does not expand into an unsolicited audit or a broader search after that point.

## Dispatch and Recovery

Only root dispatches. Keep at most one worker active for this user task; do not start a speculative backup while an execution is running. Call `list_agents` immediately before spawn; check the active tool's availability, independence requirements and capacity. Queue if capacity alone is exhausted. Identify useful independent manager work when the native tool requires it. Do not claim a dispatch that never returned an agent ID. When the same environment/capability failure recurs without new evidence or a changed condition, stop and report the blocker instead of re-probing or installing tools.
